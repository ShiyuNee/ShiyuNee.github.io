...
**关系**：
```
Q(s,a) = E[G_t | s_t=s, a_t=a]    # Q 是 G 的期望
V(s)   = E_a[Q(s,a)]               # V 是 Q 对所有动作的期望
A(s,a) = Q(s,a) - V(s)             # Advantage = 比平均好多少
```
...
> 在优化的时候，我们优化的是 policy 的参数 θ，它改变的是 P(a|s)，即一个状态下模型该有什么行为。如果在 s 下做 a 可以得到 reward，那我们应该提高 P(a|s) 的概率，而不是优化外部的 reward。
...
其中 τ 是一条完整轨迹 (s₁,a₁,r₁,s₂,a₂,r₂,...)，P(τ|θ) 是在策略 π_θ 下这条轨迹出现的概率。

**Step 2：求梯度**

```
∇J(θ) = Σ_τ ∇P(τ|θ) × R(τ)
       = Σ_τ P(τ|θ) × ∇log P(τ|θ) × R(τ)     # 利用 ∇f = f × ∇log f
       = E_τ~π_θ [ ∇log P(τ|θ) × R(τ) ]
```
...
一条轨迹的概率：
```
P(τ|θ) = P(s₁) × π_θ(a₁|s₁) × P(s₂|s₁,a₁) × π_θ(a₂|s₂) × ...
```

取 log 后：
```
log P(τ|θ) = log P(s₁) + Σ_t [log π_θ(a_t|s_t) + log P(s_{t+1}|s_t,a_t)]
```

求导时，环境转移概率 P(s_{t+1}|s_t,a_t) 和初始状态 P(s₁) 与 θ 无关，梯度为 0：
```
...
```
∇J(θ) = E_τ~π_θ [ Σ_t ∇log π_θ(a_t|s_t) × R(τ) ]
```

> 注意：求期望时天然要求是 on-policy（从 π_θ 中采样），因为公式中的期望是对 π_θ 的分布求的。
...
```
E_τ [ ∇log π_θ(a_t|s_t) × r_k ]
```

关键观察：r_k 只依赖于 (s_k, a_k, s_{k+1})，即 t 时刻之前的历史。而 ∇log π_θ(a_t|s_t) 依赖于 (s_t, a_t)。

将期望按条件分解——先对 t 时刻及之后的变量求期望，再对之前的变量求期望：

```
E_τ [ ∇log π_θ(a_t|s_t) × r_k ]
= E_{τ_{<t}} [ r_k × E_{a_t|s_t} [ ∇log π_θ(a_t|s_t) ] ]
```

其中内层期望为：
```
E_{a_t ~ π_θ(·|s_t)} [ ∇log π_θ(a_t|s_t) ]
= Σ_a π_θ(a|s_t) × ∇log π_θ(a|s_t)
= Σ_a π_θ(a|s_t) × ∇π_θ(a|s_t) / π_θ(a|s_t)
= Σ_a ∇π_θ(a|s_t)
= ∇ Σ_a π_θ(a|s_t)
= ∇ 1
= 0
...
**核心引理**：`E_{a~π}[∇log π(a|s)] = 0`，即 score function 的期望恒为零。这是因为概率分布归一化（所有概率之和为 1，其梯度为 0）。

因此，无论 r_k 取什么值，整个表达式都等于 0。对所有 k < t 的 r_k 求和仍为 0：
```
E_τ [ ∇log π_θ(a_t|s_t) × R_{<t} ] = Σ_{k<t} E_τ [ ∇log π_θ(a_t|s_t) × r_k ] = 0
```

> **推论**：任何只依赖于 s_t（而不依赖 a_t）的函数 b(s_t) 都满足 E[∇log π(a_t|s_t) × b(s_t)] = 0。这就是为什么 baseline V(s) 不影响梯度期望——它和 R_{<t} 一样，与 a_t 的选择无关。

**Step 3：用 Q 替换 G**

对于相同的 (s_t, a_t)，不同轨迹会给出不同的 G_t（因为后续的随机性）。如果采样不够多，G_t 的估计会很不稳定。

**命题**：用 Q(s_t, a_t) = E[G_t | s_t, a_t] 替换 G_t 不影响梯度的期望。
...
对于每个时刻 t，考虑单项：
```
E_τ [ ∇log π_θ(a_t|s_t) × G_t ]
```

利用条件期望的 tower property（全期望公式）：先对 (s_t, a_t) 条件化，再对 (s_t, a_t) 求期望：
```
= E_{s_t, a_t} [ ∇log π_θ(a_t|s_t) × E[G_t | s_t, a_t] ]
```

这一步的关键在于：∇log π_θ(a_t|s_t) 在给定 (s_t, a_t) 后是确定值（不再是随机变量），可以提到内层条件期望外面。而 E[G_t | s_t, a_t] 正是 Q^π(s_t, a_t) 的定义。因此：
```
= E_{s_t, a_t} [ ∇log π_θ(a_t|s_t) × Q^π(s_t, a_t) ]
```
...
E_{s_t, a_t} [ ∇log π_θ(a_t|s_t) × Q^π(s_t, a_t) ]
= E_{s_t, a_t} [ ∇log π_θ(a_t|s_t) × E[G_t | s_t, a_t] ]
= E_τ [ ∇log π_θ(a_t|s_t) × G_t ]    # tower property 反向
```
...
```
∇J(θ) = E_{s,a ~ π_θ} [ Σ_t ∇log π_θ(a_t|s_t) × Q^π(s_t, a_t) ]
```
...
```
E_{s_t, a_t ~ π_θ} [ ∇log π_θ(a_t|s_t) × V(s_t) ] = 0
```

将期望拆分为先对 a_t 求期望，再对 s_t 求期望：
```
= E_{s_t} [ V(s_t) × E_{a_t ~ π_θ(·|s_t)} [ ∇log π_θ(a_t|s_t) ] ]
```

V(s_t) 不依赖 a_t，可以提到内层期望外面。而内层期望正是 Step 2 中证明过的 score function 期望：
```
E_{a_t ~ π_θ(·|s_t)} [ ∇log π_θ(a_t|s_t) ] = 0    # Score function 引理
```
...
```
∇J(θ) = E_{s,a ~ π_θ} [ Σ_t ∇log π_θ(a_t|s_t) × A^π(s_t, a_t) ]
```
...
```
其中 G_t = r_t + r_{t+1} + ... 是从 t 时刻开始的未来回报。
...
其中 `p(x)/q(x)` 就是 importance weight（重要性权重）。两个式子的期望是相同的（无偏）。
...
```
∇J(θ) = E_{τ~π_old} [ (π_θ(a|s) / π_old(a|s)) × ∇log π_θ(a|s) × A(s,a) ]
```

从梯度推导出优化目标（对 ∇log π 积分）：

```
J(θ) = E_{s~π_old, a~π_old} [ (π_θ(a|s) / π_old(a|s)) × A(s,a) ]
```
...
```
J_PPO = E[ min(ratio × A, clip(ratio, 1-ε, 1+ε) × A) ]
```
...
```python
loss = -J_PPO = max(-ratio × A, -clip(ratio) × A)
```
...
| | Clip | KL Loss |
|--|------|---------|
| 约束对象 | 当前模型 vs 采样模型（π_new vs π_old） | 当前模型 vs 原始模型（π_new vs π_ref） |
...
- **V(s)**：在确定的 policy π 下，遇到状态 s 之后的**期望**收益
- **Q(s,a)**：在确定的 policy π 下，在状态 s 下采取行动 a 之后的**期望**收益
...
```
Q(s_t, a_t) = E[r_t + γ·V(s_{t+1})]
```

因此 Advantage 可以写成：
```
A(s_t, a_t) = Q(s_t, a_t) - V(s_t)
            ≈ r_t + γ·V(s_{t+1}) - V(s_t)    # 用单个样本的 r_t 和 s_{t+1} 近似期望
```
...
```
δ_t = r_t + γ·V(s_{t+1}) - V(s_t)              # 单步 TD Error
A_t = δ_t + γλ·δ_{t+1} + (γλ)²·δ_{t+2} + ...  # 加权求和
```
...
- `λ = 0`：只用单步 TD，`A_t = δ_t`。方差最小，偏差最大。越依赖模型已有的期望评估（偏差大，方差小），因为模型估计比较稳定但可能不准。
- `λ = 1`：等价于蒙特卡洛，`A_t = G_t - V(s_t)`。无偏差，方差最大。越依赖具体的轨迹（偏差小，方差大）。
- `0 < λ < 1`：在偏差和方差之间权衡。
...
Returns 就是对一个具体轨迹的 G(t) 的估计，用来修正 Critic 估计的 V(t)。因为 Q = A + V，V 是 Q 的期望，单个样本上的 Q 可以修正 V（因为 Q 里面带真实的 reward）。

#### 为什么 GAE 能加速梯度传播

在 LLM 中，通常只有最后一个 token 有真实的 reward（整个 response 的评分）。如果只用单步 TD：
- 第一轮更新：只有最后一个 token 受到 reward 影响
- 第二轮更新：倒数第二个 token 才开始受影响
- ...

用 GAE（λ > 0）可以让 reward 信号一次性传播到所有 token，大幅加速训练。
...
1. 初始化策略 π，并与环境交互，得到采样的训练数据
2. 利用这些数据，学习 value function V（Critic）
3. 根据 V，利用 Policy Gradient 来优化策略 π（Actor）
4. 以此循环
...
直觉：
- advantage > 0（好动作）→ loss 为负 → 最小化 loss = 增加 log π = 增加概率
- advantage < 0（差动作）→ loss 为正 → 最小化 loss = 减小 log π = 降低概率

**Step 2：加入 Importance Sampling（Off-policy）**

```
loss = -ratio × advantage
     = -(π_new / π_old) × advantage
```
...
```
loss = max(-ratio × A, -clip(ratio, 1-ε, 1+ε) × A)
```
...
| Loss | `-log P(目标token)` | `-advantage × log P(采样token)` |
...
在计算 KL(π_new || π_ref) 时，理论公式无法精确计算，需要用采样估计：
...
> **On-policy 下 ratio=1，loss 是否和当前模型无关？** 这是一个常见的困惑点。当 on-policy 第一次 forward 时，`ratio = π_new / π_old ≈ 1`，所以 `loss ≈ -advantage`。但这**并不意味着 loss 和模型参数无关**：虽然 ratio 系数≈1，但 `loss = -ratio × advantage` 中的 advantage 是常数（从 rollout 时计算好的），而梯度是通过 `ratio = exp(new_log_prob - old_log_prob)` 中的 `new_log_prob` 对模型参数 θ 求导得到的。即 `∇loss/∇θ = -∇(ratio)/∇θ × advantage = -ratio × ∇log π_θ(a|s) × advantage`。当 ratio≈1 时，`∇loss/∇θ ≈ -∇log π_θ(a|s) × advantage`——这正是 REINFORCE 的梯度公式。所以 loss 的**值**看起来和模型无关（≈-advantage），但 loss 的**梯度**完全依赖当前模型的概率分布。或者说：`loss = -ratio × advantage` 中的 `ratio` 是一个关于 θ 的函数，不是常数 1；只是在这一步，ratio 的**取值**碰巧是 1。
